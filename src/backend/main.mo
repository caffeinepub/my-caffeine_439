...
actor {
  ...
  let ADMIN_PASSWORD_HASH : Text = "@dminBGWS2001";

  func checkAdminPassword(password : Text) {
  ...
  public query func getComplaintByNumber(complaintNumber : Text) : async Complaint {
  ...
  public query func getAllComplaintsWithPassword(password : Text) : async [Complaint] {
    checkAdminPassword(password);
    complaints.values().toArray().sort();
  };

  public query func getComplaintStatsWithPassword(password : Text) : async ComplaintStats {
    checkAdminPassword(password);
    var total = 0;
    var pending = 0;
    var resolved = 0;
  ...
  public shared func updateComplaintStatusWithPassword(password : Text, complaintNumber : Text, newStatus : Complaint.Status, description : Text) : async () {
    checkAdminPassword(password);
    let complaint = switch (complaints.get(complaintNumber)) {
      case (null) { Runtime.trap("Complaint not found") };
      case (?c) { c };
  ...
  public shared func updateComplaintWithPassword(password : Text, complaintNumber : Text, input : ComplaintEditInput) : async () {
    checkAdminPassword(password);
    let complaint = switch (complaints.get(complaintNumber)) {
      case (null) { Runtime.trap("Complaint not found") };
      case (?c) { c };
  ...
  public shared func assignOfficerWithPassword(password : Text, complaintNumber : Text, officer : Text, department : Text) : async () {
    checkAdminPassword(password);
    let complaint = switch (complaints.get(complaintNumber)) {
      case (null) { Runtime.trap("Complaint not found") };
      case (?c) { c };
  ...
  public shared func addOfficerRemarksWithPassword(password : Text, complaintNumber : Text, remarks : Text, nextStep : Text) : async () {
    checkAdminPassword(password);
    let complaint = switch (complaints.get(complaintNumber)) {
      case (null) { Runtime.trap("Complaint not found") };
      case (?c) { c };
  ...
  public shared func addNoticeWithPassword(password : Text, input : NoticeInput) : async Text {
    checkAdminPassword(password);
    let noticeId = Time.now().toText();
    let notice : NoticeV2 = {
      title = input.title;
  ...
  public shared func deleteNoticeWithPassword(password : Text, noticeId : Text) : async () {
    checkAdminPassword(password);
    if (not notices2.containsKey(noticeId)) {
      Runtime.trap("Notice not found");
    };
  ...
  public shared func addNewsWithPassword(password : Text, input : NewsInput) : async Text {
    checkAdminPassword(password);
    newsCounter += 1;
    let newsId = "NEWS" # newsCounter.toText() # "_" # Time.now().toText();
    let news : News.News = {
  ...
  public shared func deleteNewsWithPassword(password : Text, newsId : Text) : async () {
    checkAdminPassword(password);
    if (not newsItems.containsKey(newsId)) {
      Runtime.trap("News not found");
    };
